...
class WalletController extends UpcController
{
    ...
    public function transferManual($input){
        ...
        if($type == 1){
            $tmp = TmpBankAuto::where('id', $tmp_id)
    ...
    public function transfer(Request $request){
        ...
        */
        ...
        // Created Order Log
        $order = $this->createOrder($data_post);
        $json = json_encode($order, JSON_UNESCAPED_UNICODE);

        // Update status wallet
        $repository->updateEntity(['response_order_at' => datetime(), 'response_order_result' => $json], $wallet);
        ...
        */
        if($order['codeid'] == 601){
            ...
            // Update status wallet
            $repository->updateEntity(['transaction_id' => $transaction_id, 'order_id' => null, 'status_id' => 7], $wallet);
            ...
            */
            ...
            return $this->success(0, ['code' => 601, 'msg' => parent::CODEID[601].$order['message'], 'ref' => $entity['ref'], 'amount' => $entity['amount'], 'created_at' => $entity['created_at'], 'order' => $order, 'data_wallet' => $data_wallet, 'transfer' => $transfer, 'void' => $void]);
        }

        // Log will creat when order create
        $log = $order['data']['log'];
        ...
        if($order['codeid'] == 602){
            // Update status wallet
            $repository->updateEntity(['transaction_id' => $transaction_id, 'order_id' => $log['order_id']], $wallet);
        ...
        */
        // Update status wallet
        $repository->updateEntity(['transaction_id' => $transaction_id, 'order_id' => $log['order_id'], 'status_id' => 3], $wallet);

        $code = 0;
        return $this->success(0, compact('code','order', 'data_wallet', 'transfer'));
