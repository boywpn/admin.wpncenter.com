...
@section('custom_body')
    ...
    <div class="row">
        <div class="col-md-12">
            <form class="form-horizontal" action="{{ route('report.winloss.game.search', $gameCode) }}" method="get">
                {{--@csrf--}}
                ...
                <div class="row clearfix">
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            <div class="form-line">
                                <input type="text" name="username" id="username" class="form-control datepicker" value="{{ (isset($request['username'])) ? $request['username'] : "" }}">
                            </div>
                        </div>
                    </div>
                </div>
    ...
    @php
        ...
        if($formData['role'] == "ag"){
            ...
            $menu .= '<li><a href="'.route('report.winloss.game', $gameCode)."?".$alink.'">WPN</a></li>';
                    ...
                    <tbody>
                        ...
                        @if(count($dataWinloss) > 0)
                            @foreach($dataWinloss as $k => $list)
                                ...
                                @if($ag->partner_id != $partner_id)
                                    @if($partner_id != 0)
                                        <tr class="total-partner">
                                            ...
                                            <th class="text-right">{{ number_format(array_sum($total_stack_count_ss[$partner_id]), 2) }}</th>
                                ...
                                <tr>
                                    <td><a href="{{ route('report.winloss.game', $gameCode) }}?role=ag&user={{ $username }}&id={{ $id }}&{{ $urlParam }}">{{ $username }}</a></td>
                                    <td>{{ $contact }}</td>
